...
static void
gaspard_print_operand_address (FILE *file, machine_mode, rtx x)
{
  switch (GET_CODE (x))
    {
    case REG:
	...
	case CONST:
	  {
		...
		fprintf (file,"+%disp(%s)", INTVAL (XEXP (plus, 1)),
...
static rtx
gaspard_function_arg (cumulative_args_t cum_v, const function_arg_info &arg)
{
  CUMULATIVE_ARGS *cum = get_cumulative_args (cum_v);
...
static void
gaspard_function_arg_advance (cumulative_args_t cum_v,
			    const function_arg_info &arg)
{
  CUMULATIVE_ARGS *cum = get_cumulative_args (cum_v);

  *cum = (*cum < gaspard_R6
	  ? *cum + ((3 + gaspard_FUNCTION_ARG_SIZE (arg.mode, arg.type)) / 4)
	  : *cum);
}
...
static bool
gaspard_pass_by_reference (cumulative_args_t, const function_arg_info &arg)
{
...
static int
gaspard_arg_partial_bytes (cumulative_args_t cum_v, const function_arg_info &arg)
{
  CUMULATIVE_ARGS *cum = get_cumulative_args (cum_v);
  int bytes_left, size;
  ...
  if (gaspard_pass_by_reference (cum_v, arg))
    size = 4;
  else if (arg.type)
    {
...
static rtx
gaspard_static_chain (const_tree ARG_UNUSED (fndecl_or_type), bool incoming_p)
{
  rtx addr, mem;
...
bool
gaspard_offset_address_p (rtx x)
{
  x = XEXP (x, 0);
...
/* Helper function for `gaspard_legitimate_address_p'.  */

static bool
gaspard_reg_ok_for_base_p (const_rtx reg, bool strict_p)
{
  int regno = REGNO (reg);
			    ...
			    addr_space_t as)
{
  ...
  if (GET_CODE(x) == PLUS
      && REG_P (XEXP (x, 0))
      && gaspard_reg_ok_for_base_p (XEXP (x, 0), strict_p)
      && CONST_INT_P (XEXP (x, 1))
      && IN_RANGE (INTVAL (XEXP (x, 1)), -32768, 32767))
    return true;
  if (REG_P (x) && gaspard_reg_ok_for_base_p (x, strict_p))
    return true;
...
#define TARGET_ADDR_SPACE_LEGITIMATE_ADDRESS_P	gaspard_legitimate_address_p
